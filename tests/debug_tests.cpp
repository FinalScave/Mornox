...
void TestDebugService() {
    ...
    const mornox::BreakpointId breakpoint = session.Context().Debug().AddBreakpoint({
    ...
    int events = 0;
    mornox::DebugSession debug_session = session.Context().Debug().Start(session.Context(), {
        .id = "debug.main",
        .name = "Debug Main",
        .type = "native",
    }, [&](const mornox::DebugEvent&) {
        ++events;
    });
    REQUIRE(debug_session.status == mornox::DebugSessionStatus::Running);
    REQUIRE(events == 1);
    ...
    REQUIRE(session.Context().Debug().Session(debug_session.id)->status == mornox::DebugSessionStatus::Paused);
    REQUIRE(session.Context().Debug().Stop(debug_session.id));
    REQUIRE(session.Context().Debug().Session(debug_session.id)->status == mornox::DebugSessionStatus::Stopped);
    session.Close();
}
