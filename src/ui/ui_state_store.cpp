...
void UiStateStore::EnsureLayoutState() {
    ...
    if (context_->CurrentProject()->GetComponent(LayoutStateStore::kComponentId) == nullptr) {
        context_->BindComponent(std::make_unique<LayoutStateStore>());
    }
    context_->RegisterContribution({
        .kind = PluginRegistrationKind::Component,
        .id = LayoutStateStore::kComponentId,
        .title = "Layout State",
        .plugin_id = "vanta.ui",
    });
}
