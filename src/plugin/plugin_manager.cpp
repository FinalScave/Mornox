...
bool MetadataOnlyRegistration(PluginRegistrationKind kind) {
    switch (kind) {
...
void BasicExtensionContext::ContributeComponent(ComponentContribution contribution) {
    activation_state_->Track(workspace_.RegisterComponentContribution(std::move(contribution)));
}
...
void PluginManager::RegisterExternalPluginCapabilities(
    ...
    for (const Value& item : result["registrations"].AsArray()) {
        const auto registration = ParsePluginRegistration(item);
        if (!registration || registration->id.empty()) {
            continue;
        }

        PluginRegistration contribution = *registration;
        contribution.plugin_id = manifest.extension.id;
        contribution.metadata_only = MetadataOnlyRegistration(registration->kind);
        activation_state.Track(workspace.RegisterContribution(std::move(contribution)));
        ...
        case PluginRegistrationKind::Component:
            break;
        case PluginRegistrationKind::Command:
            activation_state.Track(workspace.Commands().RegisterCommand(registration->id, [&host, plugin_id = manifest.extension.id, command_id = registration->id](const Value& arguments) {
