...
std::string CMakeTargetNodeId(const CMakeTarget& target) {
    return "mornox.cmake.target:" + SanitizeId(target.name);
}
...
public:
    std::string Id() const override {
        return "mornox.cmake.projectProvider";
    }
...
public:
    std::string Id() const override {
        return "mornox.cmake.projectViewProvider";
    }
    ...
    std::vector<ProjectViewNode> TopLevelNodes(WorkspaceContext& context, const ProjectView&) override {
        ...
        if (!model->graph.targets.empty()) {
            nodes.push_back(CMakeGroupNode("mornox.cmake.targets", "Targets"));
        }
        if (!model->graph.source_files.empty()) {
            nodes.push_back(CMakeGroupNode("mornox.cmake.sources", "Source Files"));
        }
        if (!model->graph.include_directories.empty()) {
            nodes.push_back(CMakeGroupNode("mornox.cmake.includes", "Include Directories"));
        }
        if (!model->graph.generated_files.empty()) {
            nodes.push_back(CMakeGroupNode("mornox.cmake.generated", "Generated Files"));
        }
        return nodes;
    }
...
public:
    void Activate(ExtensionContext& context) override {
        ...
        workspace.Settings().RegisterNode({.id = "build.cmake", .parent_id = "build", .owner_id = "mornox.cmake", .title = "CMake", .order = 10});
        workspace.Settings().RegisterSetting({
            .id = "cmake.build_directory",
            .owner_id = "mornox.cmake",
            .node_id = "build.cmake",
