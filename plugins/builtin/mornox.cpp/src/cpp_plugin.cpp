...
public:
    ...
    RunResult Run(RunExecutionContext& context, const RunConfiguration& configuration) const override {
        ...
        const std::filesystem::path run_directory = context.workspace.CurrentWorkspace().Info().root_path / ".mornox" / "run";
